...
class WalletAccountSpec: XCTestCase {
  ...
  func testCanRequestSignIn() async throws {
    ...
    try await self.walletAccount.requestSignIn(contractId: "signInContract",
                                               title: "signInTitle", presentingViewController: viewController)
  ...
  func testCompleteSignIn() async throws {
    ...
    let url = URL(string: "\(APP_SCHEME)://success?account_id=near.account&public_key=\(public_key)")!
    try await self.walletAccount.completeSignIn(url: url)
